...
$(window).on('scroll',function(){$('section').each(function(){if($(window).scrollTop()+50>=$(this).offset().top){var id=$(this).attr('id');$('.menu-item').removeClass('active');$('.menu-item.'+id).addClass('active');$('.mobile-menu-item').removeClass('active');$('.mobile-menu-item.'+id).addClass('active');}});});$('.nav-stopper').waypoint({handler:function(direction){$('.menu').toggleClass('menu-fix');$('.menu-container').toggleClass('menu-normal');$('.menu-item').toggleClass('menu-item-transparent');$('.desktop-menu .hvr-underline-from-left').toggleClass('dark');$('.mobile-menu').toggleClass('mobile-menu-fix');var a=$('.menu-link').attr('class');if(direction=='up'&&a=='menu-link active'){$('.menu-link').trigger('click');}}});$('.mobile-menu a').on('click',function(){$('.menu-link').toggleClass('active');$('.menu-slider').slideToggle(500);});$('.skill h3').waypoint({handler:function(direction){if(direction=='up'){$('.skillbar').each(function(){$(this).find('.skillbar-bar').css('width','0');});}else if(direction=='down'){$('.skillbar').each(function(){$(this).find('.skillbar-bar').animate({width:jQuery(this).attr('data-percent')},2000);});}},offset:'bottom-in-view'});const options={layout:'horizontal',setupControls:true};var filterizd=$('.filtr-container').filterizr();var morefilterizd=$('.filtr-container-detail',options).filterizr();$('.project-navigation li').on('click',function(){$('.project-navigation li').removeClass('active');$(this).addClass('active');});$('.pop-up-img').magnificPopup({type:'image',delegate:'a',gallery:{enabled:true},zoom:{enabled:true,duration:300,easing:'ease-in-out'}});$('.shuffle-stopper').waypoint({handler:function(direction){if(direction=='down'){filterizd.filterizr('shuffle');morefilterizd.filterizr('shuffle');}},offset:'bottom-in-view'});$('#container').imagesLoaded(function(){});$('.owl-carousel').owlCarousel({items:1,loop:true,dots:true,autoplay:true,autoplayTimeout:3000});var vh=$(window).height();if(navigator.userAgent.indexOf('Opera Mini')!=-1){work.start();happyClient.start();projects.start();coffee.start();$('.skillbar').each(function(){$(this).find('.skillbar-bar').animate({width:jQuery(this).attr('data-percent')},0);});$('input').removeClass('form-control');$('input').css({width:'100%',height:'50px',background:'#fff'});$('.navigation-icon').css('display','none');}
